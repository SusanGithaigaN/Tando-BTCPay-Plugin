    ...
    public override async Task<PhoneVerificationResult> ValidateMobileNumber(string msisdn, string idType, string idNumber)
    {
        ...
        try
        {
            ...
            if ((int)response.StatusCode >= 500)
            {
                ...
                return new PhoneVerificationResult(Matches: false, ServiceError: true, Detail: $"Daraja returned HTTP {(int)response.StatusCode}.");
            }

            if (!response.IsSuccessStatusCode)
            {
                logger.LogWarning("[Daraja] Client error {StatusCode}: {Content}", (int)response.StatusCode, content);
                return new PhoneVerificationResult(Matches: false, ServiceError: false, Detail: $"Daraja returned HTTP {(int)response.StatusCode}.");
            }
            ...
            return new PhoneVerificationResult(Matches: matches, ServiceError: false, Detail: json["responseMessage"]?.Value<string>());
        }
        catch (Exception ex)
        {
