...
/// <summary>Validate both adapters before the server accepts traffic. A mock configuration
...
public sealed class TandoProviderModeGuard(IConfiguration configuration, IHostEnvironment environment) : IHostedService
{
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        Validate(configuration["Tando:Payments:Mode"] ?? "Splice", "Splice", environment);
