...
public class DevelopmentVerificationTests
{
    ...
    public async Task ExplicitMockNeedsNoDarajaDependenciesAndOnlyAcceptsTheFixture()
    {
        ...
        Assert.False((await provider.ValidateMobileNumber("254702345678", "01", "mock-verified")).Matches);
    ...
    public async Task BothHttpActionsUseDevelopmentFixtures(string id, int expected)
    {
        using var services = Build("Development", "Mock");
        var controller = new TandoOnboardingController(null!, null!, null!, services.GetRequiredService<PhoneVerificationProvider>());
        var signup = await controller.Signup(new TandoSignupRequest { PhoneNumber = "0701234567", IdNumber = id }, CancellationToken.None);
        var subscribe = await controller.Subscribe(new TandoSubscribeRequest { PhoneNumber = "0701234567", IdNumber = id, PlanId = "test" }, CancellationToken.None);
        Assert.Equal(expected, Assert.IsAssignableFrom<ObjectResult>(signup).StatusCode);
        Assert.Equal(expected, Assert.IsAssignableFrom<ObjectResult>(subscribe).StatusCode);
    }
